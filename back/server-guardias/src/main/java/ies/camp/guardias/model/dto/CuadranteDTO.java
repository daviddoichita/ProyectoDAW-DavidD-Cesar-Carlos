...
public class CuadranteDTO {
    ...
    public static CuadranteDTO convertToDTO(Cuadrante cuadrante) {
        ...
        Set<SesionDTO> sesionesDTO = cuadrante.getSesiones() == null ? null
                : cuadrante.getSesiones().stream().map(SesionDTO::convertToDTO).collect(Collectors.toSet());
    ...
    public static Cuadrante convertToEntity(CuadranteDTO cuadranteDTO) {
        Cargo cargo = cuadranteDTO.getCargoDTO() == null ? null : CargoDTO.convertToEntity(cuadranteDTO.getCargoDTO());
        Set<Sesion> sesiones = cuadranteDTO.getSesionesDTO() == null ? null
                : cuadranteDTO.getSesionesDTO().stream().map(SesionDTO::convertToEntity).collect(Collectors.toSet());
